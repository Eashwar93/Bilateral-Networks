# A script based on cv for augmentation of available dataset specific to semantic segmentation
import random
import math
...
class RamdomResizedCrop(object):
    ...
    def __call__(self, im_lb):
        ...
        assert im.shape[:2] == lb.shape[:2]  # check if the resolution is matching between the image array and label array

        crop_h, crop_w = self.size
        scale = np.random.uniform(min(self.scales), max(self.scales)) # random scale between 0.5 and 1.0
        im_h, im_w = [math.ceil(el * scale) for el in im.shape[:2]] # resolution setting based on random scaling factor #shape[0] = height of the image shape[1] = width of the image
...
class ToTensor(object):
    ...
    def __call__(self, im_lb):
        im, lb = im_lb['im'], im_lb['lb']
        im = im.transpose(2, 0, 1).astype(np.float32)   # (H x W x C) to (C x H x W)
        im = torch.from_numpy(im).div_(255)
        dtype, device = im.dtype, im.device
        mean = torch.as_tensor(self.mean, dtype=dtype, device=device)[:, None, None]  # create a tensor with each mean value only on the channel dimension
        std = torch.as_tensor(self.std, dtype=dtype, device=device)[:, None, None] # create a tensor with each SD value only on the channel dimension
        im = im.sub_(mean).div_(std).clone()  # elementwise subtraction of mean and division by standard deviation
        if not lb is None:
