...
from .resnet_fanet18_v1 import Resnet18
...
class FastAttModule(nn.Module):
    def __init__(self, in_chan, mid_chn=256, out_chan=128, res=(7,10), *args, **kwargs):
        ...
        self.up = ConvBNReLU(in_chan=in_chan, out_chan=mid_chn, ks=1, stride=1, padding=1, activation='leaky_relu')

        self.smooth = ConvBNReLU(in_chan=in_chan, out_chan=out_chan, ks=3, stride=1, padding=1, activation='leaky_relu')

        self.gap = nn.AvgPool2d(kernel_size=res)
...
class SqueezeExcitationModule(nn.Module):
    def __init__(self, in_chan_res, in_chan_fam=128 ,res=(120,160), merge=True, *args, **kwargs):
        ...
        if self.merge:
            ...
            self.latlayer3 = ConvBNReLU(in_chan=in_chan_res, out_chan=in_chan_res, ks=1, stride=1, padding=0, activation='leaky_relu')
            self.up = ConvBNReLU(in_chan=in_chan_res, out_chan=int(in_chan_res/2), ks=1, stride=1, padding=1, activation='leaky_relu')
    ...
    def forward(self, feat, up_feat_in):
        ...
        if self.merge:
            ...
            concat_feat = self.latlayer3(scaled_feat)
            concat_feat =self.up(concat_feat)
        return concat_feat
...
class FPNOutput(nn.Module):
    def __init__(self, in_chan, mid_chan, n_classes, *args, **kwargs):
        ...
        self.conv2 = ConvBNReLU(in_chan=in_chan, out_chan=in_chan, ks=3, padding=1, activation='leaky_relu')
        self.conv3 = ConvBNReLU(in_chan=in_chan, out_chan=mid_chan, ks=3, padding=1, activation='leaky_relu')
...
class FANet18_v1_se3_c1(nn.Module):
    def __init__(self, n_classes=2, backbone='resnet18', aux_output=False, export=False):
        ...
        self.sem_64 = SqueezeExcitationModule(in_chan_res=512, res=(7,10), merge=False)
        self.fam_32 = FastAttModule(in_chan=256, mid_chn=256, out_chan=128, res=(15,20))
        self.fam_16 = FastAttModule(in_chan=128, mid_chn=256, out_chan=128, res=(30,40))
        self.fam_8 = FastAttModule(in_chan=64, mid_chn=256, out_chan=128, res=(60,80))
    ...
    def forward(self, x, lbl=None):
        ...
        feat4, feat8, feat16, feat32, feat64 = self.resnet(x)

        up_feat_64 = self.sem_64(feat64, None)
        upfeat_32, smfeat_32 = self.fam_32(feat32, up_feat_64, True, True)
        ...
        if self.aux_output:
            aux0 = self.aux0(up_feat_64, H, W)
            aux1 = self.aux1(smfeat_32, H, W)
            output = self.clslayer(x, H, W)
            return output, aux1, aux0
