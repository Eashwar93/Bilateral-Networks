...
class ConvX(nn.Module):
    def __init__(self, in_planes, out_planes, kernel=3, stride=1):
        ...
        self.conv = nn.Conv2d(in_channels=in_planes, out_channels=out_planes, kernel_size=kernel, stride=stride, padding=kernel//2, bias=False)
...
class AddBottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, block_num=3, stride=1):
        ...
        assert block_num > 1, print("block_num should be greater than 1")
        self.conv_list = nn.ModuleList()
        self.stride = stride
        if stride == 2:
            self.avd_layer = nn.Sequential(
                nn.Conv2d(in_channels=out_planes//2, out_channels=out_planes//2, kernel_size=3, stride=2, padding=1, groups=out_planes//2, bias=False),
                nn.BatchNorm2d(out_planes//2),
            )
            self.skip = nn.Sequential(
                nn.Conv2d(in_channels=in_planes, out_channels=in_planes, kernel_size=3, stride=2, padding=1, groups=in_planes, bias=False),
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        feat2, feat4, feat8, feat16, feat32 = net(x)
        torch.cuda.synchronize()
        start_ts = time.time()
        for i in range(100):
            feat2, feat4, feat8, feat16, feat32  = net(x)
    ...
    feat2, feat4, feat8, feat16, feat32 = net(x)
    print("Output size 8: ", feat8.size())
    print("Output size 16: ", feat16.size())
    print("Output size 32: ", feat32.size())